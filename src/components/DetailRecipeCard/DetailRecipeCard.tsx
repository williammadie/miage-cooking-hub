...
const DetailPage: React.FC<FullRecipe> = (recipeData: FullRecipe) => {
  ...
  return (
    <div
      className={`container ${
        darkMode
          ? "background-dark color-font-light"
          : "color-font-dark seconde-background-light"
      }`}
    >
      <button onClick={recipeData.goBackAction} className={"goBackButton"}>
        <img
          className={` ${
            darkMode ? "inverse-color-image" : "light-color-image"
          } `}
          src={goBackIcon}
          alt={"back button"}
        />
      ...
      <h1
        className={`title ${
          darkMode ? "primary-color-dark " : "primary-color"
        }`}
      >
        {recipeData.name}
      </h1>
